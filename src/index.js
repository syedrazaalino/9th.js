 ...
 *
 * The .ts entry point (src/index.ts) is kept in sync for editor type-awareness;
 * bundlers should resolve this .js entry point at build time.
 */
...
//     or falls back to a synchronous WebGLRenderer otherwise. The returned
//     WebGPURenderer is already initialized — callers can call `.render()`
//     directly on it without awaiting another init().
...
export function createRenderer(canvas, options = {}) {
  if (options && options.preferWebGPU) {
    // Async path — returns a Promise<WebGPURenderer|WebGLRenderer>.
    return (async () => {
      let webgpuOk = false;
      try {
  ...
  // Sync path — returns a WebGLRenderer directly (backward compatible).
  return new WebGLRenderer({ canvas, ...options });
}
